...
SIDE_BY_SIDE = {("Stamp Tools", "stamp_off", "stamp_on"), ("Path Tools", "sel_reads", "sel_writes")}

# --- inline actions (no external .py files needed) ---
...
def _add_fix_suffix(path):
    path = path.replace('\\', '/')
    dirname = os.path.dirname(path)
    basename = os.path.basename(path)
    for regex, handler in [
        (r'\.(%0(\d+)d)', lambda m: '_fix.' + m.group(1)),
        (r'\.(%d)',        lambda m: '_fix.' + m.group(1)),
        (r'\.(#+)',        lambda m: '_fix.' + m.group(1)),
        (r'\.(\$F\d*)',    lambda m: '_fix.' + m.group(1)),
    ]:
...
def _create_write_from_read():
    ...
    for read in selected:
        ...
        if not file_path:
            ...
            continue
        out_path = _add_fix_suffix(file_path)
...
INLINE_ACTIONS = {
    ...
    "convert_paths_to_relative": lambda _: _convert_paths_to_relative(),
    "create_write_from_read": lambda _: _create_write_from_read(),
    "write_reading_label": lambda _: _write_reading_label(),
}
...
class ToolboxWindow(QtWidgets.QDialog):
    ...
    def _run(self, mod_name):
        action = INLINE_ACTIONS.get(mod_name)
        if not action:
            return
        try:
            nodes = nuke.selectedNodes()
